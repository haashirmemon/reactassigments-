...
function Productslist() {
    ...
    useEffect(() => {
      ...
      const filteredArr = post.filter((data) => {
        ...
        const matchesPrice =
          ...
          (price === '150' && data.price <= 150) || 
          (price === '200' && data.price <= 200) ||
          (price === '300' && data.price <= 300) ||
          (price === '500' && data.price <= 500) ||
          (price === '1000' && data.price <= 1000);
    ...
    return (
      <>
        <h1 className='mt-4 mb-6 font-serif  text-center text-blue-600'>Products assigment by using react js</h1>
        <input
          placeholder="Search"
          type="search"
